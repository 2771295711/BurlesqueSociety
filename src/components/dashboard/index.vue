<template>
  <div class="dashBoardContain">
    ...
    <div class="another-text">
      <span v-for="(item, index) in dashBoardInfo"
            :key="index"
...
export default {
  ...
  methods: {
    handClickText(val) {
      ...
      this.$el.querySelector(`.another-text span[data-value="${val}"]`).classList.add('active');
      this.$emit("handCheckDashBoard", val);
    },
    handleMouseOver(val) {
      const targetSpan = this.$el.querySelector(`.another-text span[data-value="${val}"]`);
